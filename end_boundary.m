...
[m,n] = size(I_low);
% On cherche la colonne la plus à droite qui contient des pixels blancs
low_j = 2000;
low_i = 2000;
for j = n:-1:1
    numBlackPixels = sum(I_low(:, j) < 10);
    if numBlackPixels < m*9/10
        low_j = j
        break;
    end
end
% On cherche la ligne où sur la colonne trouvée ci-dessus, 1/2 des pixels
% blancs sont en dessous de cette ligne.
count = 0;
intensity_level = 20;
for i = m:-1:1
    if I_low(i, low_j) > intensity_level
        count = count +1;
    end
    if count > sum(I_low(:,low_j)>intensity_level)*1/3 % ou 1/4
        low_i = i
        break;
    end
end
pixel_scale = 100;
low_j = low_j - pixel_scale;
